...
class Plugin {
  ...
  render(container) {
    ...
    asteroidButton.innerHTML = 'Auto All to planets'
    asteroidButton.onclick = () => {
      sendToPlanet()
      if(autoSendToPlanet == null) {
        autoSendToPlanet = setInterval(sendToPlanet, 1000 * 60 * 15)
        message.innerText = 'Started auto all to planets';
      } else {
        message.innerText = 'Already auto all to planets';
      }
    }

    let clearAutoSendAllBtn = document.createElement('button');
    clearAutoSendAllBtn.style.width = '100%';
    clearAutoSendAllBtn.style.marginBottom = '10px';
    clearAutoSendAllBtn.innerHTML = 'Clear auto send to all';
    clearAutoSendAllBtn.onclick = () => {
      clearInterval(autoSendToPlanet)
      message.innerText = `Auto send to all Stopped`;
    ...
    withdrawtButton.innerHTML = 'Auto Withdraw from space rift';
    withdrawtButton.onclick = () => {
      withdrawSilverFunc()
      if (autoWithdraw == null) {
        autoWithdraw = setInterval(withdrawSilverFunc, 1000 * 60 * 30)
        message.innerText = `Auto Withdraw Started`;
      } else {
        message.innerText = `Auto Withdraw already running`;
      }
    }

    let clearWithdrawtButton = document.createElement('button');
    clearWithdrawtButton.style.width = '100%';
    clearWithdrawtButton.style.marginBottom = '10px';
    clearWithdrawtButton.innerHTML = 'Clear auto withdraw';
    clearWithdrawtButton.onclick = () => {
      clearInterval(autoWithdraw)
      message.innerText = `Auto Withdraw Stopped`;
      autoWithdraw = null
    }
    ...
    container.appendChild(clearAutoSendAllBtn);
    container.appendChild(toSpaceRiftButton);
    container.appendChild(withdrawtButton);
    container.appendChild(clearWithdrawtButton);
