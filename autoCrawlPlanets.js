...
class Plugin {
  ...
  render(container) {
    ...
    const doCrawlAll = () => {
      message.innerText = 'Auto Crawling, Please wait...';
      let moves = 0;
      for (let planet of df.getMyPlanets()) {
        setTimeout(() => {
          moves += capturePlanets(
          ...
          message.innerText = `Auto Crawling ${moves} planets.\n${new Date().toLocaleString()}`;
        }, 0);
      }
    }
    ...
    autoButton.onclick = () => {
      ...
      if (autoCrawlInterval == null) {
        ...
        autoButton.innerHTML = 'Auto Crawling Started'
        autoButton.disabled = true
        clearAutoButton.disabled = false
      } else {
        message.innerText = "Auto Crawling already running"
      }
      
    }

    let clearAutoButton = document.createElement('button');
    clearAutoButton.disabled = true
    clearAutoButton.style.width = '100%';
    clearAutoButton.style.marginBottom = '10px';
    clearAutoButton.innerHTML = 'Stop Auto Crawl'
    clearAutoButton.onclick = () => {
      if (autoCrawlInterval != null) {
        clearInterval(autoCrawlInterval)
        autoCrawlInterval = null
        message.innerText = `Auto Crawling Stopped`
        autoButton.innerHTML = 'Auto Crawl everything!'
        autoButton.disabled = false
      } else {
        message.innerText = `Auto Crawling is not running`;
      }
    }
    ...
    container.appendChild(clearAutoButton);
